#!/usr/bin/env python3
"""
DDP Training Script for SecBERT-GNN MITRE ATT&CK Classification.

Trains the hybrid BERT + GNN model end-to-end on labeled security data.
No pretraining required - direct multi-label classification.
...
from secbert_gnn_model import SecBERTGNNModel, AttackKnowledgeGraph, create_model
...
class SecurityLogDataset(Dataset):
        ...
        with open(data_path, 'r') as f:
            for line in f:
                ...
                valid_techniques = [
                    ...
                    if t in self.kg.technique_to_idx
                ]
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        labels = self.kg.techniques_to_labels(sample['techniques'])
...
def compute_metrics(
    ...
    # Try multiple thresholds and find optimal (include lower thresholds for imbalanced data)
    best_f1 = 0
    ...
    # Top-k F1 (use top-k predictions as binary)
    ...
    for batch in progress_bar:
        ...
        with autocast(device_type=device.type, enabled=args.fp16 and device.type == 'cuda'):
            ...
            outputs = model(
                ...
                labels=None if focal_loss_fn else labels,  # Don't compute BCE if using focal loss
            )
            ...
            if focal_loss_fn:
                ...
                # Use internal BCE loss (model computes it when labels provided)
                outputs = model(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                    labels=labels,
...
def main():
    parser = argparse.ArgumentParser(description='Train SecBERT-GNN')
    ...
    if rank == 0:
        print(f"Loading knowledge graph from {args.kg_dir}...")
    kg = AttackKnowledgeGraph(args.kg_dir)
    if rank == 0:
        print(f"  - {kg.num_nodes} nodes")
        print(f"  - {kg.num_techniques} techniques")
    ...
    # Create model
    if rank == 0:
        print(f"Creating SecBERT-GNN model...")
    ...
    # Set edge index
    edge_index = kg.get_edge_index().to(device)
    ...
    if args.use_focal_loss and FOCAL_LOSS_AVAILABLE:
        if rank == 0:
            print(f"Setting up Adaptive Focal Loss...")

        # Load technique counts for class weighting
        ...
        if rank == 0:
            ...
            print(f"  - Warmup epochs: {args.focal_warmup_epochs}")
    ...
    # Create gradient scaler for mixed precision (only for CUDA)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Update focal loss gamma for this epoch
        ...
        if epoch % args.eval_interval == 0:
            ...
            if rank == 0:
                ...
                if epochs_without_improvement >= args.early_stopping_patience:
                    ...
                    print(f"No improvement in Micro F1 for {args.early_stopping_patience} epochs.")
                    break
